...
export default async function AdvisorProfilePage() {
  ...
  const advisorId = profile?.staff_no || "QL-ADV-001";
  const officeHours = profile?.office_hours || "Thursday 14:00 - 16:00";

  return (
    <div className="max-w-4xl space-y-8 animate-in fade-in duration-500">
      <header>
        <h1 className="text-2xl font-bold text-text mb-2">Advisor Profile</h1>
        <p className="text-text-muted">Manage your advisor account information and department affiliation.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 bg-surface border border-border rounded-xl p-6 text-center space-y-4 shadow-sm">
          ...
          <div className="pt-4 border-t border-border flex items-center justify-center gap-2 text-xs font-semibold text-success bg-success-bg/20 py-1.5 rounded-lg">
            <Shield className="w-4 h-4" /> Advisor Role
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-surface border border-border rounded-xl p-6 space-y-6 shadow-sm">
          <h3 className="text-lg font-bold text-text border-b border-border pb-3 flex items-center gap-2">
            <Contact className="w-5 h-5 text-primary" /> Advisor Details
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              <span className="text-xs font-semibold text-text-muted uppercase tracking-wider">Advisor ID</span>
              <div className="flex items-center gap-2 text-sm text-text font-medium bg-bg-page/50 p-3 rounded-lg border border-border">
                <Award className="w-4 h-4 text-text-muted" /> {advisorId}
              </div>
            </div>
