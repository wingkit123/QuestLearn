...
- Attributes: `user_id` (PK), `role_id` (FK), `full_name`, `email`, `password_hash`, `account_status`, `created_at`
...
- Relationships: each student profile belongs to one user; one student profile can have many enrollments, submissions, quiz attempts, and progress records.
...
- Description: stores academic advisor information used for student monitoring and follow-up.
- Relationships: each advisor profile belongs to one user; one advisor can monitor students within their assigned department.
...
- Attributes: `lesson_id` (PK), `module_id` (FK), `lesson_title`, `lesson_type`, `content_text`, `video_url`, `publish_status`
- Description: represents a learning unit inside a module. Lesson content such as reading material and embedded video is stored directly within the lesson record.
- Relationships: one module can have many lessons; one lesson can have many quizzes, assignments, and progress records.
...
- Attributes: `notification_id` (PK), `user_id` (FK), `announcement_id` (FK, optional), `message`, `is_read`, `sent_at`
- Description: stores an in-app notification delivered to a user, such as deadline reminders, new content alerts, or quiz score announcements.
- Relationships: each notification belongs to one user and may be linked to an announcement.
...
- integration of embedded videos and reading materials within lessons
...
- in-app notifications for assignment deadlines, new course content uploads, and quiz score announcements
...
This use case enables a new student or instructor to create an account and log in to access the platform.

| Field                            | Description                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Use Case Name                    | Register Account and Login                                                                                                                                                                                                                                                                                                                            |
| Actors                           | Student, Instructor                                                                                                                                                                                                                                                                                                                                               |
| Preconditions                    | The user does not already have an active account.                                                                                                                                                                                                                                                                                                  |
| Normal Flow                      | 1. The user opens the registration page. 2. The user enters required account information including name, email, student/staff ID, password, and programme/department. 3. The system validates the submitted data. 4. The system creates the account and assigns the appropriate role. 5. The user is redirected to the login page. 6. The user enters credentials and logs in. 7. The system opens the user dashboard. |
| Postconditions                   | The user account is created and the user is logged in with access to their dashboard. (Note: Instructors may require admin approval before full course creation features are unlocked).                                                                                                                                                                                                                                                            |
| Alternative Flows and Exceptions | If the email is already registered, the system shows an error and directs the user to log in. If login credentials are invalid, the system allows retry. After 3 failed login attempts, the account is locked for 15 minutes. If a user forgets their password, they contact the Admin who resets it to a temporary default value. The user must change their password upon next login. |
...
| Normal Flow                      | 1. The student opens a course. 2. The student selects a module. 3. The student selects a lesson. 4. The system displays lesson content, including reading material and embedded video. 5. The system records page visits, video interactions, and lesson access in the activity log. 6. The system updates lesson progress status. |
...
| Field                            | Description                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Use Case Name                    | Publish Lesson Content                                                                                                                                                                                                                                                                                                                                               |
| Actors                           | Instructor                                                                                                                                                                                                                                                                                                                                                           |
| Preconditions                    | A course, module, and lesson already exist.                                                                                                                                                                                                                                                                                                                          |
| Normal Flow                      | 1. The instructor selects a lesson. 2. The instructor uploads or links reading material and video content. 3. The instructor saves lesson content. 4. The instructor publishes the lesson. 5. The system makes the lesson available to enrolled students. 6. The system records content publication for notification and analytics purposes. |
...
This use case enables an academic advisor to review student performance data and perform follow-up actions.

| Field                            | Description                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Use Case Name                    | View Advisor Dashboard and Follow Up                                                                                                                                                                                                                                                                                                                                                                                                                |
| Actors                           | Academic Advisor                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Preconditions                    | The academic advisor is logged in and has students in their department.                                                                                                                                                                                                                                                                                                                                                                             |
| Normal Flow                      | 1. The advisor opens the dashboard. 2. The system displays students in the advisor's department with progress summaries, quiz scores, overdue work, and engagement indicators. 3. The advisor selects a student. 4. The system displays the student's progress history, quiz performance, and overdue assignments. 5. The advisor reviews the student's performance data. 6. The advisor sends a follow-up advisory message. 7. The system records the follow-up status. |
| Postconditions                   | The advisor has current information for follow-up, and the follow-up action is recorded.                                                                                                                                                                                                                                                                                                                                                            |
| Alternative Flows and Exceptions | If no students require attention, the advisor can still review student progress summaries.                                                                                                                                                                                                                                                                                                                                                          |
| Non-Functional Requirements      | Student performance data should be timely, easy to interpret, and protected according to appropriate access control rules.                                                                                                                                                                                                                                                                                                                          |
...
- Identity and access: `Role`, `User`, `StudentProfile`, `InstructorProfile`, `AdvisorProfile`
- Learning structure: `Course`, `Module`, `Lesson`, `Enrollment`
...
- Support and analytics: `ActivityLog`, `Announcement`, `Notification`
...
- Relationships: each student profile belongs to one user and can be linked to enrollments, submissions, attempts, and progress records.
...
- Description: stores academic advisor details used for student monitoring and follow-up.
- Relationships: each advisor profile belongs to one user; one advisor can monitor students within their assigned department.
...
- Attributes:
  ...
  - `lesson_type`: Type of lesson (e.g., video, reading, mixed).
  - `content_text`: Reading or instructional content for the lesson.
  - `video_url`: External video embed URL (e.g., YouTube).
  - `publish_status`: Visibility state for the lesson.
- Description: represents an individual lesson within a module. Lesson content such as reading material and embedded video is stored directly within the lesson record.
- Relationships: one lesson can have many quizzes, assignments, and progress records.
...
- Description: stores in-app notifications such as deadline reminders, new content alerts, and quiz score announcements.
- Relationships: each notification belongs to one user and may be linked to an announcement.
...
4. Security: Authentication and role-based access control must be enforced to protect user accounts and administrative functions.
5. Maintainability: The system should use a structured modular design so future enhancements such as deeper analytics, gamification, or email verification can be added without major redesign.
6. Compatibility: The platform should be accessible through common modern web browsers and support embedded video content.
...
The following features are outside the initial MVP scope but are designed to be added in future iterations without major architectural changes.

1. Email verification for account activation using an external email service.
2. H5P interactive content authoring and embedding through Lumi or similar tools.
3. Gamification features including badges, student badge awards, and learning streak tracking.
4. Reusable notification templates managed by admins for standardized notification delivery.
5. Stored recommendation records for rule-based learning suggestions based on weak-topic analysis.
6. Automated advisor alert generation with risk-level scoring and trigger-based early warnings.
7. Formal advisor-student assignment tracking with reassignment history.
8. Mobile-native application support and push notifications.
